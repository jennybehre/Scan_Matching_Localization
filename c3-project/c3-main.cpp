...
int main(){
	...
	lidar->Listen([&new_scan, &lastScanTime, &scanCloud](auto data){
		...
		if(new_scan){
			...
			if(pclCloud.points.size() > 7000){ // CANDO: Can modify this value to get different scan resolutions
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			if(i == 0) {
				// Init the pose at beginning of the localization. Afterwards localize with the lidar measurements
